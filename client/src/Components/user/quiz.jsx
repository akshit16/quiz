...
class quiz extends Component {
  ...
  getsubcateg = async () => {
    ...
    for (var i = 0; i < body.subcateg.length; i++) {
      if (this.state.catvalue == body.subcateg[i].category) {
        var subcategory = [];
        subcategory = Object.assign({}, body.subcateg[i], {
          category: undefined
        });
        ...
        for (var j = 0; j < subcategory.value.length; j++)
          obj[j] = {
            key: subcategory.text[j],
            text: subcategory.text[j],
            value: subcategory.value[j]
          };
        console.log(obj);
  ...
  getName = async () => {
    ...
    for (var i = 0; i < body.quizname.length; i++) {
      if (
        this.state.catvalue == body.quizname[i].category &&
        this.state.subvalue == body.quizname[i].subcategory
      ) {
        var quizName = [];
        ...
        var obj = [];
        for (var j = 0; j < quizName.length; j++) console.log(quizName.length);
        obj[j] = {
          key: quizName.text,
          text: quizName.text,
  ...
  countDown = () => {
    if (this.state.session) this.setState({ time: this.state.time - 1 });
  ...
  score = e => {
    ...
    if (this.state.i + 1 <= this.state.data.length) {
      if (this.state.data[this.state.i].answer == e.currentTarget.innerHTML) {
        ...
        if (this.state.i + 1 < this.state.data.length) {
          setTimeout(() => {
            this.setState({ i: this.state.i + 1, time: 20 });
            document.getElementById("3").style.background = this.state.color;
            document.getElementById("4").style.background = this.state.color;
            document.getElementById("1").style.background = this.state.color;
            document.getElementById("2").style.background = this.state.color;
          }, 500);
        } else {
          setTimeout(() => {
            ...
            {this.state.session && this.state.data.length > 0 && (
              <div>
                ...
                {this.state.time > 0 && this.state.data[this.state.i] != "" && (
                  <div>
                    <div>
                      <h3 style={{ float: "left", marginLeft: "10%" }}>
                    ...
                    >
                      {that.state.data[this.state.i].option1}
                    </div>
                    <div
                      className="card1"
                      id="2"
                    ...
                    >
                      {this.state.data[this.state.i].option2}
                    </div>
                    <div
                      className="card1"
                      id="3"
                    ...
                    >
                      {that.state.data[this.state.i].option3}
                    </div>
                    <div
                      className="card1"
                      id="4"
                    ...
                    >
                      {that.state.data[this.state.i].option4}
                    </div>
                  </div>
                )}
